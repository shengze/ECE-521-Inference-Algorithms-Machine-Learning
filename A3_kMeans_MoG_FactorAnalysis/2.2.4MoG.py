...
input_data = np.load('data100D.npy')
np.random.seed(521)
...
def graph():
    ...
    return train,data,mean,loss_f, assignment,tf.exp(prior_softmax),sigma_exp
    
def runMult():
    train_err = []
    valid_err = []
    # Build computation graph
    train,data,mean,loss, assignment,prior,sigma = graph()
    ...
    for i in np.arange(500):
        _, err, center, assign,prior1,sigma1= sess.run([train, loss, mean, assignment,prior,sigma], feed_dict={data:trainData})
    ...
    print('Cluster number:',K,'VALID_loss:',errValid)
    print('Cluster number:', K, 'percentage:', prior1)
    print('Cluster number:', K, 'sigma:', sigma1)
...
#    y = trainData[:, 1]
#    
#    plt.scatter(x,y,c=assign,marker='.')
K=5
