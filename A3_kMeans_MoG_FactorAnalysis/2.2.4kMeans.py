...
# data features
data_num = np.shape(trainData)[0]
data_dim = np.shape(input_data)[1]
#tf.set_random_seed(560)
...
def graph(K,n):
    ...
    return train,Loss,centorid,data,percentage
...
def runMult(K,n):
    ...
    train,loss,centorid,data ,percent= graph(K,n)
    ...
    for i in np.arange(500):
        _, err, center,p= sess.run([train, loss, centorid,percent], feed_dict={data:trainData})
        train_err.append(err)
        #print('update_times:',i,'TRAIN_loss:',err)
        
        errValid = sess.run(loss, feed_dict= {data: validData})
        valid_err.append (errValid)
    print('cluster number:',K,'percentage:',p)
    print('validation loss:',errValid)
