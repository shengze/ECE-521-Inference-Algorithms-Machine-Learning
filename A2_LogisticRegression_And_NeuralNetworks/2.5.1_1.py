...
def neural_network_model(d, nodes1,nodes2):
    ...
    output_layer = {'weights': tf.Variable(tf.random_normal([nodes2, n_classes], stddev=3./(nodes2+n_classes))),
    ...
    return output, hidden_layer_1['weights'], hidden_layer_2['weights'], output_layer['weights']
...
def train_neural_network(x):
    prediction, W1, W2, W = neural_network_model(x, n_nodes_hl1, n_nodes_hl2)
    cost = tf.reduce_mean(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(prediction,y) + 0.5*lamda * (tf.reduce_mean(tf.square(W1))+tf.reduce_mean(tf.square(W2))+tf.reduce_mean(tf.square(W)))))
    optimizer = tf.train.AdamOptimizer(0.00192).minimize(cost)
