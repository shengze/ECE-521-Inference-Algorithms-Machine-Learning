...
def neural_network_model(d, nodes1, nodes2, nodes3):

    hidden_layer_1 = {'weights': tf.Variable(tf.random_normal([784, nodes1], stddev=3./(nodes1+batch_size))),
                      'biases': tf.Variable(0.0, [nodes1,])}
    hidden_layer_2 = {'weights': tf.Variable(tf.random_normal([nodes1, nodes2], stddev=3. / (nodes1 + nodes2))),
                      'biases': tf.Variable(0.0, [nodes2,])}
    hidden_layer_3 = {'weights': tf.Variable(tf.random_normal([nodes2, nodes3], stddev=3. / (nodes2 + nodes3))),
                      'biases': tf.Variable(0.0, [nodes3, ])}

    output_layer = {'weights': tf.Variable(tf.random_normal([nodes3, n_classes], stddev=3./(nodes3+n_classes))),
    ...
    hidden_layer_2['weights'] = tf.cast(hidden_layer_2['weights'], tf.float64)
    hidden_layer_2['biases'] = tf.cast(hidden_layer_2['biases'], tf.float64)
    hidden_layer_3['weights'] = tf.cast(hidden_layer_3['weights'], tf.float64)
    hidden_layer_3['biases'] = tf.cast(hidden_layer_3['biases'], tf.float64)
    ...
    return output, hidden_layer_1['weights'], hidden_layer_2['weights'],hidden_layer_3['weights'], output_layer['weights']
...
def train_neural_network(x):
    prediction, W1, W2, W3,W = neural_network_model(x, n_nodes_hl1, n_nodes_hl2, n_nodes_hl3)
    cost = tf.reduce_mean(tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(prediction,y) + 0.5*lamda * (tf.reduce_mean(tf.square(W1))+tf.reduce_mean(tf.square(W2))+tf.reduce_mean(tf.square(W3))+tf.reduce_mean(tf.square(W)))))
    optimizer = tf.train.AdamOptimizer(0.00225).minimize(cost)
...
plt.plot(test_accuracy, label='train_curve')
plt.plot(valid_accuracy, label='test_curve')
le=plt.legend(loc='upper right', shadow=True)
plt.show()
print('final validation error = ',valid_accuracy[19])
print('final test error =',test_accuracy[19])
