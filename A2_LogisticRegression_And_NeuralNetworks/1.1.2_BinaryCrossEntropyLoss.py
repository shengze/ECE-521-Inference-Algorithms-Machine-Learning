...
def buildGraph(N, lamda):
    ...
    # Error definition
    sigmoidCrossEntropyError = tf.reduce_mean(
        tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(y_logits, y_target),
                       name='sigCroEntroError') + lamda * tf.reduce_sum(tf.square(W)) / float(2), name='mean_error')
...
def runMult(N):
    ...
    for step in range(0, 100):
        ...
        for index in range(0, int(3500 / batch_size)):
            ...
            _, err, currentW, currentb, yhat_x, yhat = sess.run(
                [train, sigmoidCrossEntropyError, W, b, y_logits, y_predicted], feed_dict={X: batch1, y_target: batch2})
...
# Tuning learing rate
#train_err0= runMult(0.001)
#train_err1= runMult(0.01)
train_err4,test_err,train_accuracy,test_accuracy= runMult(0.01)
#train_err2= runMult(0.1)

#plt.plot(train_err0, label='learning rate = 0.001')
#plt.plot(train_err1, label='learning rate = 0.01')
#plt.plot(train_err4, label='learning rate = 0.05')
#plt.plot(train_err2, label='learning rate = 0.1')
#legend = plt.legend(loc='upper right', shadow=True)
#plt.show()

# PLot error curve
#plt.plot(train_err4, label='training_curve')
